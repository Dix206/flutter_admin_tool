...
class CmsAttributeColorWidget extends StatelessWidget {
  final Color? currentValue;
  final String title;
  final OnCmsTypeUpdated<Color> onCmsTypeUpdated;
  final bool shouldDisplayValidationErrors;
  final CmsAttributeColor cmsAttributeColor;

  const CmsAttributeColorWidget({
    Key? key,
    required this.title,
    required this.currentValue,
    required this.onCmsTypeUpdated,
    required this.shouldDisplayValidationErrors,
    required this.cmsAttributeColor,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      title: Text(
        cmsAttributeColor.valueToString(
          context: context,
          value: currentValue,
        ),
      ),
      ...
      subtitle: shouldDisplayValidationErrors && !cmsAttributeColor.isValid(currentValue)
          ? Text(
              cmsAttributeColor.invalidValueErrorMessage ?? cmsTexts.defaultInvalidDataMessage,
    ...
    return AlertDialog(
      content: Column(
        ...
        children: [
          ...
          Text(cmsTexts.cmsAttributeColorHexTitle),
          const SizedBox(height: 4),
          TextField(
            controller: _hexController,
            decoration: InputDecoration(
              hintText: cmsTexts.cmsAttributeColorHexTitle,
